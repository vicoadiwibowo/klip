...
echo -e "${YELLOW}[2/6]${NC} Install paket dasar (python, ffmpeg, git)..."
pkg install -y python ffmpeg git 2>&1 | tail -2 || true
...
pip install --quiet --upgrade flask requests 2>&1 | grep -v "already satisfied" | tail -2 || true
pip install --quiet --upgrade yt-dlp 2>&1 | grep -v "already satisfied" | tail -2 || true
echo -e "${GREEN}  OK${NC}"
